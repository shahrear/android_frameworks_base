...
#define GPUFREQ_NODE "/sys/devices/platform/pvrsrvkm.0/sgx_dvfs_cur_clk"
...
static jobjectArray native_GetTemperature(JNIEnv* env, jobject obj) {
	...
	for (int i = 0; i < 13; i++) {
		if (buf[i] != ' ')
			value[core_idx][start_idx++] = buf[i];
		else {
			start_idx = 0;
			env->SetObjectArrayElement(ret, core_idx, env->NewStringUTF(value[core_idx]));
			core_idx++;
		}
	}
...
#if 0
char *current_sensoer_node_list[128] = {
	"/sys/devices/platform/exynos5-hs-i2c.0/i2c-4/4-0040/sensor_V",
	"/sys/devices/platform/exynos5-hs-i2c.0/i2c-4/4-0040/sensor_A",
	"/sys/devices/platform/exynos5-hs-i2c.0/i2c-4/4-0040/sensor_W",

	"/sys/devices/platform/exynos5-hs-i2c.0/i2c-4/4-0045/sensor_V",
	"/sys/devices/platform/exynos5-hs-i2c.0/i2c-4/4-0045/sensor_A",
	"/sys/devices/platform/exynos5-hs-i2c.0/i2c-4/4-0045/sensor_W",

	"/sys/devices/platform/exynos5-hs-i2c.0/i2c-4/4-0044/sensor_V",
	"/sys/devices/platform/exynos5-hs-i2c.0/i2c-4/4-0044/sensor_A",
	"/sys/devices/platform/exynos5-hs-i2c.0/i2c-4/4-0044/sensor_W",

	"/sys/devices/platform/exynos5-hs-i2c.0/i2c-4/4-0041/sensor_V",
	"/sys/devices/platform/exynos5-hs-i2c.0/i2c-4/4-0041/sensor_A",
	"/sys/devices/platform/exynos5-hs-i2c.0/i2c-4/4-0041/sensor_W",
};


static jobjectArray native_GetAVW(JNIEnv* env, jobject obj) {
	FILE *fp = NULL;
	...
	for (int i = 0; i < 12; i++) {
		fp = fopen(current_sensoer_node_list[i], "r");
		if (fp == NULL) {
			ALOGE("Not opened Sensor");
			return NULL;
		}
		if ( i % 3 == 0)
			fread(&value[index++], 1, 3, fp);
		else
			fread(&value[index++], 1, 5, fp);
		fclose(fp);
	}

	jobjectArray ret = (jobjectArray)env->NewObjectArray(12, env->FindClass("java/lang/String"), 
			env->NewStringUTF(""));

	for (int i = 0; i < 12; i++) {
		env->SetObjectArrayElement(ret, i, env->NewStringUTF(value[i]));
	}
	return ret;
}
#endif
