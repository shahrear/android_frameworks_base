...
void SensorService::MplSysConnection::onFirstRef()
{
    ...
    if(pgsiof == NULL) {
        ALOGE("could not find symbol for getSysInterfaceObject");
    }

    sys_iface = pgsiof();
    if(sys_iface != NULL) {
        ALOGV("getSysInterfaceObject found at %p", &sys_iface);
    }
}
...
{
    ALOGV("Test MPL Sys Connection");
    return 4242;
}

status_t SensorService::MplSysConnection::getBiases(float *f)
{
    ALOGV("server side getBiases %p %p %p", mplSysApi_l, sys_iface->getBiases, f);
...
void SensorService::MplSysPedConnection::onFirstRef()
{
    ...
    if(pgsiof == NULL) {
        ALOGE("could not find symbol for getSysPedInterfaceObject");
    } else {
        ALOGV("getSysPedInterfaceObject found at %p", mplSysApi_l);
    }
...
void SensorService::MplConnection::onFirstRef()
{
    ...
    if(mplGlyphApi_l == NULL) {
        ALOGE("could not find symbol for mplGlyphApi");
    } else {
        ALOGV("mplGlyphApi found at %p", mplGlyphApi_l);
    }
}
