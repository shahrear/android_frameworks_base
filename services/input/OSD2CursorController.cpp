...
#define WINDOS_AXIS_PATH     "/sys/class/graphics/fb0/window_axis"
#define SCALE_MODE_PATH      "/sys/class/graphics/fb0/freescale_mode"
...
int set_sysfs_str(const char *path, const char *val)
{
    ...
    if (fd >= 0) {
        ...
        ALOGI("OSD2CursorController set_sysfs_str %s= %s\n", path,val);
        close(fd);
        return 0;
    } else {
    }
...
void OSD2CursorController::setScaleAxis()
{
    char ppscaler_rect[80] = {0}; 
    char scale_mode[SYSCMD_BUFSIZE] = {0}; 
    char osd1_rect[SYSCMD_BUFSIZE] = {0};
    char osd2_axis[SYSCMD_BUFSIZE] = {0}; 
    char display_axis[SYSCMD_BUFSIZE] = {0}; 
    int x,y,w,h;
    int ret = 0;


    get_sysfs_str(SCALE_MODE_PATH, scale_mode, sizeof(scale_mode));
    if(strstr(scale_mode,"new")!=NULL){
        get_sysfs_str(WINDOS_AXIS_PATH, osd1_rect, sizeof(osd1_rect));
        ret = sscanf(osd1_rect, "window axis is [%d %d %d %d]", &x, &y, &w, &h);
    }else{
        get_sysfs_str(PPMGR_SCALE_RECT, ppscaler_rect, sizeof(ppscaler_rect));
        ret = sscanf(ppscaler_rect, "ppscaler rect:\nx:%d,y:%d,w:%d,h:%d", &x, &y, &w, &h);
    }

    if( ret == 4) {
        snprintf(osd2_axis, SYSCMD_BUFSIZE, "1920 1080 %d %d",  w-x+1,  h-y+1);
        set_sysfs_str(FB1_SCALE_PATH, "0x0");
        set_sysfs_str(FB1_SCALE_AXIS_PATH, osd2_axis);
        set_sysfs_str(FB1_SCALE_PATH, "0x10001");
        snprintf(display_axis, SYSCMD_BUFSIZE, "0 0 1920 1080 %d %d 18 18",  x, y);
        set_sysfs_str(DISPLAY_AXIS_PATH, display_axis);
    }
}
