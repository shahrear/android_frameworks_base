...
public class EthernetStateTracker implements NetworkStateTracker {
    ...
    private boolean configureInterface(EthernetDevInfo info) throws UnknownHostException {
        ...
        if (info.getConnectMode().equals(EthernetDevInfo.ETH_CONN_MODE_DHCP)) {
            if(!mEM.isEthDeviceUp()) {
                Slog.d(TAG, "device is not up!");
                return false;
            }
            synchronized(mDhcpTarget) {
                if (!mStartingDhcp) {
        ...
        public boolean handleMessage(Message msg) {
            synchronized (this) { //TODO correct 'this' object?
                ...
                case EVENT_HW_PHYCONNECTED:
                    ...
                    Slog.i(TAG, "New status, stackConnected=" + mStackConnected + " HWConnected=" + mHWConnected+" mStartingDhcp="+mStartingDhcp);
                    if (!mStartingDhcp) {
                        int state = mEM.getEthState();
                        if (state != mEM.ETH_STATE_DISABLED) {
                            info = mEM.getSavedEthConfig();
            ...
            case EVENT_DHCP_START:
                synchronized (mDhcpTarget) {
                    if (!mInterfaceStopped) {
                        ...
                        dhcpFlag = true;
                        result = NetworkUtils.runDhcp(mInterfaceName, mDhcpResults);
                ...
                // getSavedEthConfig() require EthernetService lock
                // resetInterface() called by EthernetService.UpdateEthDevInfo(), keep EthernetService lock,  and requirc mDhcpTarget lock
                // so to avoid dead lock, move getSavedEthConfig() out of synchronized (mDhcpTarget) 
                if(dhcpFlag) {
                    if (result) {
                        event = EVENT_INTERFACE_CONFIGURATION_SUCCEEDED;
                        ...
                        if(DhcpError.contains("dhcpcd to start") || DhcpError.contains("Timed out waiting for DHCP to finish") ){
                            ...
                            if(mHWConnected){
                                Slog.i(TAG, "mHWConnected is true, configureInterface !!!");
                                int state = mEM.getEthState();
                                if (state != mEM.ETH_STATE_DISABLED) {
                                    info = mEM.getSavedEthConfig();
                                    if(info != null){
                                    ...
                                    Slog.i(TAG, "mHWConnected is false");
                            }
                        }
                        else 
                            event = EVENT_INTERFACE_CONFIGURATION_FAILED ;

                        String	prop_val = SystemProperties.get("net.dhcp.repeat", "enabled");
