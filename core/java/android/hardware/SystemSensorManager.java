...
public class SystemSensorManager extends SensorManager {
    ...
    protected boolean registerListenerImpl(SensorEventListener listener, Sensor sensor,
        ...
        synchronized (mSensorListeners) {
            ...
            if(mContext!=null && result && Sensor.TYPE_ACCELEROMETER == sensor.getType() && hasAccelerometer){
                String curPkgName = mContext.getApplicationInfo().packageName;
                int versionCode = -1;
                try {
                        versionCode = mContext.getPackageManager().getPackageInfo(curPkgName, 0).versionCode;
                } catch (Exception e) {
                        e.printStackTrace();
                }
                Log.i(TAG,"registerListenerImpl curPkgName:" + curPkgName + " versionCode:" + versionCode);
              
                ContentValues  values = new ContentValues();
                ContentResolver  resolver =  mContext.getContentResolver();
                Cursor c = resolver.query(CONTENT_URI, null, "AKG_NAME="+ "'"+ curPkgName+ "'", null, null);
                if(c!=null&&c.moveToFirst()){
                    mWhitePackage = c.getInt(3);
                }else{
                    mWhitePackage = 0x0000;
                }
                
                String gSensorConfig = SystemProperties.get("persist.sys.gsensor","off");
                if(gSensorConfig.equals("on")) {
                    registerMonitor();
                ...
                Log.d(TAG,"====mWhitePackage:"+mWhitePackage);
                /*
                SensorAccessPkgCfg cfg = null;
                if( mSensorAccessList != null ) {
                    String strKey = curPkgName + "," + "-1";//if all version match
                    if(mSensorAccessList.containsKey(strKey)) {
                        cfg = mSensorAccessList.get(strKey);
                    } else {
                        strKey = curPkgName + "," + versionCode;
                        if(mSensorAccessList.containsKey(strKey)) {//if special version match
                            cfg = mSensorAccessList.get(strKey);
                        }
                    }
                }
                ...
                if( cfg != null ) {
                    mWhitePackage = cfg.mSensorConfig;
                }
                else {
                    mWhitePackage = 0x0000;
                }
        ...
        synchronized (mSensorListeners) {
            ...
            if (queue != null) {
                ...
                if (result && !queue.hasSensors()) {
                    ...
                    if ( mContext != null && mMonitorStart ){
                        Intent intent = new Intent("android.intent.action.gSensorBroadcast");
                        intent.putExtra("show",false);
                        Log.d(TAG,"===unregisterListenerImpl  sendBroadcast");
                        mContext.sendBroadcast(intent);
                        //return;
                    }
